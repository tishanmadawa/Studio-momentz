            ...
            <div id="container" class="container">
                ...
                <div class="row">
                    <div class="col-md-9 col-md-offset-0">
                        <h2 class="text-center">Edit Package Details</h2>
                        <br><br>
                        <?php
                            //if(isset($view_data) && is_array($view_data) && count($view_data)): $i=1;
                            foreach ($edit_data as $key => $data) { 
                        ?>                          
                        <form method="post" action="<?php echo site_url('Controller/update_data/'.$data['Package'].''); ?>"> 
                            
                            
                            <label for="Package">Package Name</label>
                            <input type="text" class="form-control" name="Package" value="<?php echo $data['Package']; ?>" required >
                            <br>
                            <label for="Price">Price</label>
                            <input type="text" class="form-control" name="Price" value="<?php echo $data['Price']; ?>" required>
                            <br>
                            <label for="NoofCameramen">No of Photoes</label>
                            <input type="text" class="form-control" name="NoofCameramen" value="<?php echo $data['NoofPhotoes']; ?>" required>
                            <br>
                            <label for="NoofPhotoes">No of Camera men</label>
                            <input type="text" class="form-control" name="NoofPhotoes" value="<?php echo $data['NoofCameramen']; ?>" required>
                            <br>
                            <label for="Flashes">Flashes </label>
                            <input type="text" class="form-control" name="Flashes" value="<?php echo $data['Flashes']; ?>" required>
                            <br>
                            <label for="VideoHours">Video Hours </label>
                            <input type="text" class="form-control" name="VideoHours" value=" <?php echo $data['VideoHours']; ?>" required>
                            <br>
